...
class TrackerController extends Controller
{
    public function index(): Response
    {
        $trackers = Tracker::query()
            ...
            ->map(fn (Tracker $t) => [
                ...
                'ranklists_count' => $t->rank_lists_count,
            ]);
    ...
    private function transformTrackerData(Tracker $tracker): array
    {
        return [
            'id' => $tracker->id,
            'title' => $tracker->title,
    ...
    private function transformRankListData(RankList $selectedRankList): array
    {
        return [
            ...
            'events' => $this->transformEventsData($selectedRankList),
            'users' => $this->transformUsersData($selectedRankList),
        ];
    }

    private function transformEventsData(RankList $selectedRankList): array
    {
        return $selectedRankList->events->map(function ($event) use ($selectedRankList) {
            $data = [
                'id' => $event->id,
                'title' => $event->title,
    ...
    private function transformUsersData(RankList $selectedRankList): array
    {
        return $selectedRankList->users->map(function ($user) {
