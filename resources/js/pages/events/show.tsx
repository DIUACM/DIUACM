...
function AttendanceModal({
    ...
    return (
        <>
            <Button onClick={handleAttendanceClick} className="w-full bg-blue-600 text-white hover:bg-blue-700">
                        ...
                        <div className="space-y-6 text-center">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="flex flex-col gap-3 pt-2">
                                    <Button type="submit" className="h-11 w-full bg-blue-600 text-white hover:bg-blue-700" disabled={processing}>
                                        {processing ? (
                                            <>
    ...
    if (attendees.length === 0) {
        return (
            <div className="py-12 text-center text-slate-500 dark:text-slate-400">
                ...
                <h3 className="mb-2 text-lg font-medium">No attendees yet</h3>
                ...
                {attendees.map((attendee) => (
                    ...
                    >
                        <Avatar className="h-10 w-10 flex-shrink-0">
                            ...
                            <AvatarFallback className="bg-blue-100 text-sm font-medium text-blue-700 dark:bg-blue-900 dark:text-blue-300">
    ...
    if (users.length === 0) {
        return (
            <div className="py-12 text-center text-slate-500 dark:text-slate-400">
                ...
                <h3 className="mb-2 text-lg font-medium">No performance data yet</h3>
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1 flex items-center gap-3">
                                <div className="flex items-center gap-1 text-xs">
                                    ...
                                    <span className="font-medium text-green-700 dark:text-green-400">{user.solves_count}</span>
                                    <span className="text-slate-500 dark:text-slate-400">solves</span>
                                </div>
                                <div className="flex items-center gap-1 text-xs">
                                    <Target className="h-3 w-3 text-blue-600 dark:text-blue-400" />
                                    <span className="font-medium text-blue-700 dark:text-blue-400">{user.upsolves_count}</span>
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="mb-8">
                    <Button
                        onClick={goBack}
                        variant="ghost"
                        className="mb-4 -ml-2 text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white"
                    >
                    ...
                    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                        <div className="flex-1">
                            <h1 className="mb-2 text-3xl font-bold text-slate-900 dark:text-white">{event.title}</h1>
                            <div className="flex flex-wrap items-center gap-3">
                                <StatusBadge start={event.starting_at} end={event.ending_at} />
                                <Badge variant="secondary" className="capitalize">
                                    {typeLabel(event.type)}
                                </Badge>
                                <Badge variant="outline">{scopeLabel(event.participation_scope)}</Badge>
                            </div>
                        </div>
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <p className="font-medium">{formatDate(event.starting_at)}</p>
                                    </div>
                                    <div>
                                        <Label className="text-sm font-medium text-slate-600 dark:text-slate-300">Time</Label>
                                        <p className="font-medium">{formatTimeRange(event.starting_at, event.ending_at)}</p>
                                    </div>
                                </div>
                                ...
                                {event.description && (
                                    <div>
                                        ...
                                        <div
                                            className="prose prose-sm dark:prose-invert max-w-none leading-relaxed text-slate-700 dark:text-slate-200"
                                ...
                                {event.event_link && (
                                    <div>
                                        <Label className="text-sm font-medium text-slate-600 dark:text-slate-300">Event Link</Label>
                                        <a
                                ...
                                {event.rank_lists && event.rank_lists.length > 0 && (
                                    <div>
                                        <Label className="text-sm font-medium text-slate-600 dark:text-slate-300">Rated for</Label>
                                        <div className="mt-1 flex flex-wrap gap-2">
                                            {event.rank_lists.map((rankList) => (
                                                <div
                                                    key={rankList.id}
                                                    className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-100 px-3 py-1.5 dark:border-slate-700 dark:bg-slate-800"
                                                >
                                                    ...
                                                    <span className="text-xs font-medium text-slate-500 dark:text-slate-400">{rankList.weight}x</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                <Card>
                                    <CardContent className="p-0">
                                        <Tabs defaultValue="performance" className="w-full">
                                            ...
                                            <TabsContent value="performance" className="mt-6 px-6 pb-6">
                                                <UsersWithStatsList users={event.users_with_stats} />
                                            </TabsContent>
                                            <TabsContent value="attendance" className="mt-6 px-6 pb-6">
