            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="mb-6">
                    <Link href={`/internal-contests/${registration.internal_contest.slug}`}>
                        <Button variant="ghost" className="gap-2 pl-0 transition-all hover:pl-2">
                ...
                <div className="grid gap-8 lg:grid-cols-3">
                    ...
                    <div className="space-y-6 lg:col-span-2">
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl dark:text-gray-100">My Registration</h1>
                                <p className="mt-1 text-base text-muted-foreground">{registration.internal_contest.title}</p>
                            </div>
                        ...
                        {/* Status Banner */}
                        <StatusBanner status={registrationStatus} isConfirmed={isConfirmed} payment={payment} hasFee={hasFee} />
                        {/* Registration Details Card */}
                        <Card>
                            <CardHeader>
                                <div className="flex items-center justify-between">
                                    <CardTitle className="text-lg">Registration Details</CardTitle>
                                    <span className="rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium">#{registration.id}</span>
                                </div>
                                <CardDescription>Registered {formatDate(registration.registered_at)}</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                {/* Personal Information */}
                                <div className="space-y-3">
                                    <h3 className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Personal Information</h3>
                                ...
                                <div className="space-y-3">
                                    <h3 className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Academic Information</h3>
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        ...
                                        <InfoItem icon={Users} label="Gender" value={<span className="capitalize">{registration.gender}</span>} />
                                        {registration.lab_teacher_name && (
                                            <InfoItem icon={Users} label="Lab Teacher" value={registration.lab_teacher_name} />
                                        )}
                                    </div>
                                </div>
                                ...
                                {(registration.tshirt_size || registration.pickup_point) && (
                                    <>
                                        <Separator />
                                        <div className="space-y-3">
                                            <h3 className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Event Logistics</h3>
                                            <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        {registration.payment_history.length > 0 && (
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>
                                        {registration.payment_history.length} payment{' '}
                                        {registration.payment_history.length === 1 ? 'attempt' : 'attempts'}
                                    </CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="space-y-3">
                                        {registration.payment_history.map((paymentItem, index) => (
                                            <PaymentHistoryItem key={paymentItem.id} payment={paymentItem} isLatest={index === 0} />
                                        ))}
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        <div className="sticky top-8 space-y-6">
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-3">
                                    <StatusBadge label="Registration" status={registrationStatus} type="registration" />
                                    {hasFee && payment && <StatusBadge label="Payment" status={payment.status} type="payment" />}
                                </CardContent>
                            </Card>
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div>
                                            ...
                                            <p className="text-2xl font-bold">৳{registration.internal_contest.registration_fee}</p>
                                        </div>
                                        ...
                                        {showPaymentButton && (
                                            <>
                                                <Separator />
                                                <Button onClick={handlePayment} className="w-full" disabled={form.processing}>
    ...
    if (status === 'under_review' || payment?.status === 'under_manual_review') {
        return (
            <div className="flex items-start gap-3 rounded-lg border border-purple-200 bg-purple-50 p-4 dark:border-purple-900 dark:bg-purple-900/20">
                ...
                <div className="flex-1">
                    <h3 className="font-semibold text-purple-900 dark:text-purple-100">Under Manual Review</h3>
                    ...
                    {payment?.transaction_id && (
                        <p className="mt-2 font-mono text-xs text-purple-600 dark:text-purple-400">Reference: {payment.transaction_id}</p>
                    )}
                </div>
            </div>
        );
    ...
    if (payment?.status === 'pending') {
        return (
            <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-900 dark:bg-blue-900/20">
                ...
                <div className="flex-1">
                    <h3 className="font-semibold text-blue-900 dark:text-blue-100">Payment Processing</h3>
    ...
    if (payment?.status === 'failed' || payment?.status === 'canceled') {
        return (
            <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-900/20">
                ...
                <div className="flex-1">
                    <h3 className="font-semibold text-red-900 dark:text-red-100">Payment {payment.status === 'failed' ? 'Failed' : 'Canceled'}</h3>
    ...
    if (status === 'pending' && hasFee && !payment) {
        return (
            <div className="flex items-start gap-3 rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-900 dark:bg-yellow-900/20">
                ...
                <div className="flex-1">
                    <h3 className="font-semibold text-yellow-900 dark:text-yellow-100">Payment Required</h3>
...
function StatusBadge({ label, status, type }: { label: string; status: string; type: 'registration' | 'payment' }) {
...
function PaymentHistoryItem({ payment, isLatest }: { payment: InternalContestMyRegistration['payment_history'][0]; isLatest: boolean }) {
    ...
    return (
        <div className={cn('flex items-start gap-3 rounded-lg border p-3', isLatest ? 'border-primary/50 bg-primary/5' : 'border-border')}>
            <div className={cn('mt-0.5', getStatusColor(payment.status))}>{getStatusIcon(payment.status)}</div>
            <div className="flex-1 space-y-1">
                <div className="flex items-center justify-between gap-2">
                    <span className={cn('text-sm font-medium', getStatusColor(payment.status))}>{formatStatus(payment.status)}</span>
                    {isLatest && <span className="rounded-full bg-primary px-2 py-0.5 text-xs font-medium text-primary-foreground">Latest</span>}
                </div>
                <p className="text-sm text-muted-foreground">
                    ৳{payment.amount} · {payment.gateway}
                </p>
                {payment.paid_at && <p className="text-xs text-muted-foreground">Paid {formatDate(payment.paid_at)}</p>}
