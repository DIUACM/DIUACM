...
const mapEventPerformance = (statsRows, performanceRows, warnings) => {
  const expanded = [...statsRows, ...performanceRows];
  const out = [];

  for (const row of expanded) {
    const eventId = toInt(nested(row, ["event_id", "eventId"]));
    const userId = toInt(nested(row, ["user_id", "userId", "programmer_id", "programmerId"]));
    if (eventId === null || userId === null) {
...
const buildImport = (payload) => {
  ...
  rows.eventPerformance = mapEventPerformance(raw.eventUserStats, raw.eventPerformance, warnings);

  const lines = [
    "-- Generated by scripts/import-structure.mjs",
    "-- Imports event_user_stats into event_performance.",
    "BEGIN TRANSACTION;",
  ];
...
const main = async () => {
  ...
  const result = buildImport(payload);
