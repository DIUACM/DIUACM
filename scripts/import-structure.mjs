...
const buildImport = (payload) => {
  ...
  const lines = [
    ...
    "-- Intentionally skips event_user_stats and event_performance.",
    "BEGIN TRANSACTION;",
  ];
  ...
  return {
    ...
    skipped: {
      event_user_stats: raw.eventUserStats.length,
      event_performance: raw.eventPerformance.length,
    },
    sql: lines.join("\n"),
    warnings,
  };
...
const printSummary = (result, sqlPath, options) => {
  ...
  console.log(`Skipped event_user_stats: ${result.skipped.event_user_stats}`);
  console.log(`Skipped event_performance: ${result.skipped.event_performance}`);
