...
const SUPPORTED_PASSWORD_HASH = /^(?:\$2[aby]\$(?:0[4-9]|1[0-5])\$[./A-Za-z0-9]{53}|pbkdf2:\d+:[0-9a-fA-F]+:[0-9a-fA-F]+)$/;
...
const aliases = {
  ...
  eventUserStats: ["event_user_stats", "eventUserStats"],
  eventPerformance: ["event_performance", "eventPerformances"],
};

const usage = `Usage:
  pnpm import:structure [--remote] [--input export.json] [--passwords-only | --performance-only] [--dry-run] [--out tmp/import.sql]
...
Options:
  ...
  --passwords-only    Update only existing users' supported password hashes.
  --performance-only  Import only event solve/upsolve performance rows.
...
const parseArgs = (argv) => {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--passwords-only") {
      options.passwordsOnly = true;
      continue;
    }
    if (
      arg === "--input" ||
  ...
  if (options.passwordsOnly && options.performanceOnly) {
    fail("--passwords-only and --performance-only cannot be used together");
  }
  return options;
};
...
const mapUsers = (rows, warnings, imageAssets) => {
  ...
  for (const row of rows) {
    ...
    // The legacy Laravel export contains bcrypt hashes in `password`. Keep the
    // hash verbatim: the API accepts it on the first login and upgrades it to
    // the current PBKDF2 format after successful verification.
    const passwordHash = textOrNull(
      direct(row, ["password_hash", "passwordHash", "password"]),
    );
...
const mapEventPerformance = (statsRows, performanceRows, warnings) => {
  const out = [];

  for (const row of [...statsRows, ...performanceRows]) {
    const eventId = toInt(nested(row, ["event_id", "eventId"]));
    const userId = toInt(
      nested(row, ["user_id", "userId", "programmer_id", "programmerId"]),
    );
    if (eventId === null || userId === null) {
      warn(warnings, "event_performance", rowId(row), "missing event_id or user_id; skipped");
      continue;
    }
    ...
    out.push({
      ...
      solve_count: toInt(nested(row, ["solve_count", "solveCount", "solves"])) ?? 0,
      upsolve_count: toInt(nested(row, ["upsolve_count", "upsolveCount", "upsolves"])) ?? 0,
      created_at:
        toEpochSeconds(nested(row, ["created_at", "createdAt"])) ?? Math.floor(Date.now() / 1000),
      updated_at:
        toEpochSeconds(nested(row, ["updated_at", "updatedAt"])) ?? Math.floor(Date.now() / 1000),
    });
  }
...
const buildPerformanceImport = (payload, options) => {
  const warnings = [];
  const raw = {
    eventUserStats: findRows(payload, aliases.eventUserStats),
    eventPerformance: findRows(payload, aliases.eventPerformance),
  };
  const rows = {
    eventPerformance: mapEventPerformance(
      raw.eventUserStats,
      raw.eventPerformance,
      warnings,
    ),
  };
  const lines = [
    "-- Generated by scripts/import-structure.mjs --performance-only",
    "-- Upserts event solve/upsolve performance without changing imported structure.",
  ];
  if (options.local) lines.push("BEGIN TRANSACTION;");

  for (const row of rows.eventPerformance) {
    lines.push(
      insertSql("event_performance", row, ["event_id", "user_id"], [
        "position",
        "solve_count",
        "upsolve_count",
        "created_at",
  ...
  return {
    ...
    rowCounts: { eventPerformance: rows.eventPerformance.length },
    imageAssets: [],
    sql: lines.join("\n"),
    warnings,
  };
...
const buildPasswordImport = (payload, options) => {
  const warnings = [];
  const rawUsers = findRows(payload, aliases.users);
  const rows = [];
  ...
  for (const row of rawUsers) {
    ...
    if (!passwordHash) {
      warn(warnings, "users", id, "missing password hash; skipped");
      continue;
    }
    if (!SUPPORTED_PASSWORD_HASH.test(passwordHash)) {
      warn(warnings, "users", id, "unsupported password hash; skipped");
      continue;
    }
    rows.push({ id, password_hash: passwordHash });
  }

  const uniqueRows = addUnique(rows, (row) => row.id);
  const lines = [
    "-- Generated by scripts/import-structure.mjs --passwords-only",
    "-- Updates password hashes for existing users without changing other data.",
  ];
  if (options.local) lines.push("BEGIN TRANSACTION;");
  for (const row of uniqueRows) {
    lines.push(
      `UPDATE \`users\` SET \`password_hash\` = ${sqlValue(row.password_hash)} WHERE \`id\` = ${sqlValue(row.id)};`,
    );
  }
  if (options.local) lines.push("COMMIT;");
  lines.push("");
  ...
  return {
    rawCounts: { users: rawUsers.length },
    rowCounts: { userPasswords: uniqueRows.length },
    imageAssets: [],
    sql: lines.join("\n"),
    warnings,
  };
...
const buildImport = (payload, options) => {
  if (options.passwordsOnly) return buildPasswordImport(payload, options);
  if (options.performanceOnly) return buildPerformanceImport(payload, options);
  ...
  for (const row of rows.users) {
    ...
    // A structure-only export may omit credentials. In that case, leave an
    // existing D1 password intact rather than clearing it on an upsert.
    if (row.password_hash !== null) updateColumns.splice(4, 0, "password_hash");
    lines.push(
      insertSql("users", row, ["id"], updateColumns),
    );
  }
